 ...
 */
uint32_t crc32_ppc(uint32_t crc, unsigned char const *data, size_t len) {
  unsigned char *buf2;
...
__crc32_vpmsum(unsigned int crc, const void* p, unsigned long len) {
	...
	if (len < 256) {
		...
		do {
			/* Checksum in blocks of MAX_SIZE_VALUE. */
			block_size = length;
			if (block_size > MAX_SIZE_VALUE) {
				block_size = MAX_SIZE_VALUE;
			}
			...
			* data - 128 / 16 = 8
			*/
			offset = (MAX_SIZE_VALUE/8) - (block_size/8);
