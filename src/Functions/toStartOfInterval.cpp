...
namespace
{
    ...
    {
        static UInt32 execute(UInt16, Int64, const DateLUTImpl &, Int64, const char* function_name) { return dateIsNotSupported(function_name); }

        static UInt32 execute(Int32, Int64, const DateLUTImpl &, Int64, const char* function_name) { return dateIsNotSupported(function_name); }
    ...
    struct Transform<IntervalKind::Millisecond>
    {
        ...
        static UInt32 execute(UInt32, Int64, const DateLUTImpl &, Int64, const char* function_name) { return dateTimeIsNotSupported(function_name); }
...
private:
    ...
    ColumnPtr execute(const FromDataType &, const ColumnType & time_column_type, Int64 num_units, const DataTypePtr & result_type, const DateLUTImpl & time_zone, const UInt16 scale) const
    {
        ...
        for (size_t i = 0; i != size; ++i)
        {
            try
            {
                result_value_data[i] = Transform<unit>::execute(time_data[i], num_units, time_zone, scale_multiplier, name);
                if constexpr (execution_error_policy == ExecutionErrorPolicy::Null)
